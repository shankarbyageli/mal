...
const EVAL = (ast, env) => {
  while(true) {
    ...
    if (func instanceof Fn) {
      ...
      if (func.isVariadic) {
        const args = evaluatedList.ast.slice(1, func.params.ast.length);
        const variadicArgs = evaluatedList.ast.slice(1 + args.length, evaluatedList.ast.length);
        env = new Env(func.env, func.params.ast, [...args, new List(variadicArgs)]);
      }
      ast = func.ast;
    } else {
